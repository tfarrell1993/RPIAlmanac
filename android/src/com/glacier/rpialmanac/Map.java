    ...
    case 50:
      if(resultCode == RESULT_OK) {
        ...
        if(res != null) {
          ...
          int iLoc = res.getInt("i");
          Location loc = locations.get(iLoc);
          loc.setRating(rating);
          ratingView.setText(loc.getRating() + " / 5 stars");
          Log.v("GLACIER", "rating: " + rating);
          Log.v("GLACIER", "iLoc: " + iLoc);
          for(HashMap.Entry<Marker,Location> entry : markerLocationMap.entrySet()) {
            if(entry.getValue().getName() == loc.getName()) {
              entry.getKey().showInfoWindow();
              Log.v("GLACIER","refreshing window");
            }
          }
        }
      }
